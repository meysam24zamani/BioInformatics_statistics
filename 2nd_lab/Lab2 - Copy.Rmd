...
saveRDS(flag, file="monomorphicVar")
flag <- readRDS("monomorphicVar")
number.of.monomorphic <-length(flag)#number of monomorphic variants

print(100*length(flag)/(number.of.variants))
```
...
respectively.
```{r}
poly.data <- subset(chr22, select = -flag) #remove the monomorphic variants from data
...
```
## [1] "We have 8162 significant SNPs with alpha =0.05"
...
print(length(chisq.stats[chisq.stats > 0.05]))
```
## [1] "We have 200912 markers out of equilibrium"
...
```
## [1] "We have 144 most significant SNPs"
...
```
## [1]"We have 5793 significant SNPs"
...
```
## [1] "We have 7955 significant SNPs with alpha 0.05"
...
Yes, they are consistent even though there are some values differents.
```{r}
...
We can see in the following plot that the right side is empty. We can see in red color the significant markers and in green color the non-significant ones which seems to be the most dense of the plot since the bottom there are not enough markers.
```{r}
...
It will not affect the statistical tests for HWE since swapping the alleles A or B does not affect to the test
since p and q are symmetrical
...
```{r}
prettyPrintStat("mean",mean(inbreeding.coff))
```
## [1] "The mean is: -0.00466823238593223"

```{r}
prettyPrintStat("standard deviation", sd(inbreeding.coff))
```
## [1] "The standard deviation is: 0.0950119986299748"

```{r}
prettyPrintStat("median", median(inbreeding.coff))
```
## [1] "The median is: -0.00469483568075117"

```{r}
prettyPrintStat("minimum", min(inbreeding.coff))
```
## [1] "The minimum is: -0.981481481481482"

```{r}
prettyPrintStat("maximum", max(inbreeding.coff))
```
## [1] "The maximum is: 1"


when ^f is equal to 0 means that HWE holds, we expect to be normally distributed with mean equal 0 and
std 0.1.
...
We can see below that decreasing the number of the alpha, the significant variants are smaller but there is no
relationship between the alpha and the number of significant variants.
```{r}
...
## [2] "The percentage of alpha 0.1 is: 4.80643217234089%"
##
## [[2]]
## [1] "We have 5793 with alpha 0.05"
## [2] "The percentage of alpha 0.05 is: 2.77078928991649%"
##
## [[3]]
## [1] "We have 2508 with alpha 0.01"
## [2] "The percentage of alpha 0.01 is: 1.1995752700001%"
##
## [[4]]
## [1] "We have 1485 with alpha 0.001"
## [2] "The percentage of alpha 0.001 is: 0.710274830921109%"
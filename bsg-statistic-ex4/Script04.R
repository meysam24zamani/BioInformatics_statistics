...
sum(apply(Z,2,is.mono))

ind.mono <- apply(Z,2,is.mono)
Z <- Z[,!ind.mono]
dim(Z)

Geno <- cbind(substr(Z[,1],1,1),substr(Z[,1],2,2))

for(i in 2:ncol(Z)) {
  Geno <- cbind(Geno,substr(Z[,i],1,1),substr(Z[,i],2,2))
}
             
Geno
Snpnames <- paste("SNP",1:ncol(Z),sep="")
Snpnames
Haplo.Res <- haplo.em(Geno,locus.label=Snpnames,control=haplo.em.control(min.posterior=1e-4))
Haplo.Res
